...
%But this is just speculation, we have not investigated the idea further. 
%This may lead to trouble, given the previous encoding.
%Consider, for instance, the regular languages $a*b*$ and $(ab)*$.
%For any length $> 2$, they accept different strings: 
%$\{aaaa, aaab, \dots, bbbb\}$ and $\{abab\}$ have no common strings in them. 
%For instance, given the language $a*b*$ and length 2, we would have 3 CGs,
%generating the strings $aa$, $ab$ and $bb$.
