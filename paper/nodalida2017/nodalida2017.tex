...
then the result will have only one \t{opt\_POS} reading per cohort,
and we proceed to check whether the original word forms correspond to those:
\t{"<det>"} for \t{opt\_det}, and so on.

If there are multiple strings of the same length, we'll have to lax our criteria:
as long as there is a corresponding \t{opt\_POS} in the cohort, such as
 \t{"<det>"} and \{\t{opt\_det}, \t{opt\_adj}\}, we accept the string.
...
This may lead to trouble: consider, for instance, the regular languages (TODO: use Pepijn's example here)
